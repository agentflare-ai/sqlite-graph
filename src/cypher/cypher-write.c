...
*/
static int
isValidLabelName (const char *zLabel)
{
  if (!zLabel || strlen (zLabel) == 0)
    return 0;
  if (strlen (zLabel) > MAX_LABEL_LENGTH)
    return 0;

  /* Must start with letter or underscore */
  if (!isalpha (zLabel[0]) && zLabel[0] != '_')
    return 0;

  /* Can contain letters, digits, underscores */
  for (int i = 1; zLabel[i]; i++)
    {
      if (!isalnum (zLabel[i]) && zLabel[i] != '_')
        return 0;
    }
...
*/
static int
isValidPropertyName (const char *zProperty)
{
  if (!zProperty || strlen (zProperty) == 0)
    return 0;
  if (strlen (zProperty) > MAX_PROPERTY_NAME_LENGTH)
    return 0;

  /* Must start with letter or underscore */
  if (!isalpha (zProperty[0]) && zProperty[0] != '_')
    return 0;

  /* Can contain letters, digits, underscores */
  for (int i = 1; zProperty[i]; i++)
    {
      if (!isalnum (zProperty[i]) && zProperty[i] != '_')
        return 0;
    }
...
*/
static int
isValidVariableName (const char *zVariable)
{
  if (!zVariable || strlen (zVariable) == 0)
    return 0;
  if (strlen (zVariable) > MAX_VARIABLE_NAME_LENGTH)
    return 0;

  /* Must start with letter or underscore */
  if (!isalpha (zVariable[0]) && zVariable[0] != '_')
    return 0;

  /* Can contain letters, digits, underscores */
  for (int i = 1; zVariable[i]; i++)
    {
      if (!isalnum (zVariable[i]) && zVariable[i] != '_')
        return 0;
    }
...
*/
static int
isReservedWord (const char *zWord)
{
  static const char *azReserved[]
      = { "CREATE",   "MERGE",  "SET",  "DELETE", "DETACH",   "MATCH",
          "WHERE",    "RETURN", "WITH", "UNWIND", "OPTIONAL", "UNION",
          "ORDER",    "BY",     "SKIP", "LIMIT",  "ASC",      "DESC",
          "AND",      "OR",     "NOT",  "XOR",    "CASE",     "WHEN",
          "THEN",     "ELSE",   "END",  "AS",     "DISTINCT", "TRUE",
          "FALSE",    "NULL",   "IN",   "IS",     "STARTS",   "ENDS",
          "CONTAINS", NULL };
...
cypherGenerateSecureNodeId (GraphVtab *pGraph)
{
...
*/
static sqlite3_int64
cypherGenerateSecureRelationshipId (GraphVtab *pGraph)
{
  static sqlite3_int64 iNextRelId = 1;
  sqlite3_int64 iRelId;

  /* Simple implementation - in production, use crypto-secure random */
  iRelId = time (NULL) * 1000 + 500000
           + (iNextRelId++); /* Offset from node IDs */

  /* Ensure ID doesn't already exist - check against edge storage */
  sqlite3_stmt *pStmt = NULL;
  char *zSql = sqlite3_mprintf (
      "SELECT 1 FROM %s WHERE id = %lld LIMIT 1", pGraph->zEdgeTableName, iRelId);
  if (!zSql)
    return -1;
      ...
      if (rc == SQLITE_ROW)
        {
          ...
          zSql = sqlite3_mprintf (
              "SELECT 1 FROM %s WHERE id = %lld LIMIT 1", pGraph->zEdgeTableName,
              iRelId);
          if (!zSql)
            return -1;
...
*/
static char *
cypherSanitizeString (const char *zInput)
{
  if (!zInput)
    return NULL;
  ...
  char *zSanitized
      = sqlite3_malloc (nLen * 2 + 1); /* Worst case: all chars escaped */
  if (!zSanitized)
    return NULL;
...
*/
static int
cypherWriteContextBeginOp (CypherWriteContext *pCtx, CypherWriteOpType type)
{
  if (!pCtx)
...
*/
static int
cypherWriteContextRollbackOp (CypherWriteContext *pCtx)
{
  if (!pCtx)
    return SQLITE_MISUSE;
  ...
  if (pCtx->pLastOp)
    {
      ...
      for (CypherWriteOp *pOp = pCtx->pOperations; pOp && pOp != pToRemove;
           pOp = pOp->pNext)
        {
          pPrev = pOp;
        }
...
*/
CypherWriteContext *
cypherWriteContextCreate (sqlite3 *pDb, GraphVtab *pGraph,
                          ExecutionContext *pExecContext)
{
  ...
  /* Seed next-id counters from storage to avoid id reuse collisions */
  if (pGraph) {
    sqlite3_int64 nNext = cypherStorageGetNextNodeId(pGraph);
    sqlite3_int64 rNext = cypherStorageGetNextEdgeId(pGraph);
    pCtx->iNextNodeId = (nNext > 0) ? nNext : 1;
    pCtx->iNextRelId  = (rNext > 0) ? rNext : 1;
  } else {
    pCtx->iNextNodeId = 1;
    pCtx->iNextRelId  = 1;
  }
...
*/
void
cypherWriteContextDestroy (CypherWriteContext *pCtx)
{
...
*/
int
cypherWriteContextBegin (CypherWriteContext *pCtx)
{
  int rc;
...
*/
int
cypherWriteContextCommit (CypherWriteContext *pCtx)
{
  int rc;
  ...
  if (!pCtx)
    return SQLITE_MISUSE;
  if (!pCtx->bInTransaction)
    return SQLITE_OK; /* Nothing to commit */
...
*/
int
cypherWriteContextRollback (CypherWriteContext *pCtx)
{
  int rc;
  ...
  if (!pCtx)
    return SQLITE_MISUSE;
  if (!pCtx->bInTransaction)
    return SQLITE_OK; /* Nothing to rollback */
...
*/
int
cypherWriteContextAddOperation (CypherWriteContext *pCtx, CypherWriteOp *pOp)
{
  if (!pCtx || !pOp)
    return SQLITE_MISUSE;
...
*/
sqlite3_int64
cypherWriteContextNextNodeId (CypherWriteContext *pCtx)
{
  if (!pCtx)
    return -1;
  if (pCtx->iNextNodeId <= 0 && pCtx->pGraph) {
    sqlite3_int64 nNext = cypherStorageGetNextNodeId(pCtx->pGraph);
    pCtx->iNextNodeId = (nNext > 0) ? nNext : 1;
  }
...
*/
sqlite3_int64
cypherWriteContextNextRelId (CypherWriteContext *pCtx)
{
  if (!pCtx)
    return -1;
  if (pCtx->iNextRelId <= 0 && pCtx->pGraph) {
    sqlite3_int64 rNext = cypherStorageGetNextEdgeId(pCtx->pGraph);
    pCtx->iNextRelId = (rNext > 0) ? rNext : 1;
  }
...
** Returns SQLITE_OK on success, error code on failure.
** Follows @SELF_REVIEW.md requirements for input validation, security, and
*error handling.
*/
int
cypherCreateNode (CypherWriteContext *pCtx, CreateNodeOp *pOp)
{
  ...
  /* Validate labels */
  if (pOp->nLabels < 0 || pOp->nLabels > 100)
    { /* Reasonable limit */
      return SQLITE_RANGE;
    }
  ...
  for (i = 0; i < pOp->nProperties; i++)
    {
      ...
      if (pOp->apPropValues[i]->type == CYPHER_VALUE_STRING
          ...
          && strlen (pOp->apPropValues[i]->u.zString)
  ...
  /* @SELF_REVIEW.md Section 5.1: Build labels JSON array with security
   * sanitization */
  if (pOp->nLabels > 0)
    {
      int nAlloc = 512;
      ...
      for (i = 0; i < pOp->nLabels; i++)
        {
          ...
          nUsed += snprintf (zLabels + nUsed, nAlloc - nUsed, "%s\"%s\"",
                             i > 0 ? "," : "", zSanitizedLabel);
          sqlite3_free (zSanitizedLabel);
        }
  ...
  /* @SELF_REVIEW.md Section 5.1: Build properties JSON object with security
   * sanitization */
  if (pOp->nProperties > 0)
    {
      ...
      for (i = 0; i < pOp->nProperties; i++)
        {
          /* @SELF_REVIEW.md Section 5.1: Sanitize property names against
           * injection */
          char *zSanitizedPropName
              = cypherSanitizeString (pOp->azPropNames[i]);
          ...
          /* @SELF_REVIEW.md Section 2.1: Handle all value types with proper
           * error checking */
          if (pOp->apPropValues[i]->type == CYPHER_VALUE_STRING)
            {
              if (pOp->apPropValues[i]->u.zString)
                {
                  zSanitizedPropValue
                      = cypherSanitizeString (pOp->apPropValues[i]->u.zString);
                  ...
                  nNeeded
                      = snprintf (NULL, 0, "%s\"%s\":\"%s\"", i > 0 ? "," : "",
                                  zSanitizedPropName, zSanitizedPropValue);
                }
              else
                {
            ...
            {
              nNeeded = snprintf (
                  NULL, 0, "%s\"%s\":%s", i > 0 ? "," : "", zSanitizedPropName,
                  pOp->apPropValues[i]->u.bBoolean ? "true" : "false");
            }
          else
            {
              nNeeded = snprintf (NULL, 0, "%s\"%s\":null", i > 0 ? "," : "",
                                  zSanitizedPropName);
            }
          ...
          if (pOp->apPropValues[i]->type == CYPHER_VALUE_STRING)
            ...
            {
              nUsed
                  += snprintf (zProps + nUsed, nAlloc - nUsed, "%s\"%s\":%lld",
                               i > 0 ? "," : "", zSanitizedPropName,
                               pOp->apPropValues[i]->u.iInteger);
            }
          else if (pOp->apPropValues[i]->type == CYPHER_VALUE_FLOAT)
            {
              nUsed += snprintf (zProps + nUsed, nAlloc - nUsed, "%s\"%s\":%g",
                                 i > 0 ? "," : "", zSanitizedPropName,
                                 pOp->apPropValues[i]->u.rFloat);
            }
          else if (pOp->apPropValues[i]->type == CYPHER_VALUE_BOOLEAN)
            {
              nUsed += snprintf (zProps + nUsed, nAlloc - nUsed, "%s\"%s\":%s",
                                 i > 0 ? "," : "", zSanitizedPropName,
                                 pOp->apPropValues[i]->u.bBoolean ? "true"
                                                                  : "false");
            }
          else
            {
              nUsed
                  += snprintf (zProps + nUsed, nAlloc - nUsed, "%s\"%s\":null",
                               i > 0 ? "," : "", zSanitizedPropName);
            }
  ...
  pWriteOp->zNewLabels
      = sqlite3_mprintf ("%s", zLabelsJson); /* Deep copy for rollback */
  ...
  /* @SELF_REVIEW.md Section 2.1: Actually add node to graph storage with error
   * handling */
  ...
  rc = cypherStorageAddNode (pCtx->pGraph, pOp->iCreatedNodeId, azLabelPtrs,
                             pOp->nLabels, zPropsJson);
  if (rc != SQLITE_OK)
    {
      sqlite3_free (zLabelsJson);
      sqlite3_free (zPropsJson);
  ...
  /* @SELF_REVIEW.md Section 1.2: Bind variable in execution context with error
   * handling */
  if (pOp->zVariable)
    {
      ...
      rc = executionContextBind (pCtx->pExecContext, pOp->zVariable,
                                 &nodeValue);
...
*/
int
cypherCreateRelationship (CypherWriteContext *pCtx, CreateRelOp *pOp)
{
  CypherWriteOp *pWriteOp = NULL;
  char *zPropsJson = NULL;
  int rc = SQLITE_OK;
  ...
  if (pOp->zFromVar
      && (!isValidVariableName (pOp->zFromVar)
          || isReservedWord (pOp->zFromVar)))
    {
      return SQLITE_FORMAT;
    }
  if (pOp->zToVar
      && (!isValidVariableName (pOp->zToVar) || isReservedWord (pOp->zToVar)))
    {
      return SQLITE_FORMAT;
    }
  if (pOp->zRelVar
      && (!isValidVariableName (pOp->zRelVar)
          || isReservedWord (pOp->zRelVar)))
    {
      return SQLITE_FORMAT;
    }
  ...
  if (pOp->nProperties > 0)
    {
      ...
      for (i = 0; i < pOp->nProperties; i++)
        {
          /* Calculate space needed for this property - account for
           * sanitization */
          int nNeeded = 0;

          if (pOp->apPropValues[i]->type == CYPHER_VALUE_STRING)
            {
              /* @SELF_REVIEW.md Section 5.1: Account for string sanitization
               * expansion */
              size_t nOrigLen = pOp->apPropValues[i]->u.zString
                                    ? strlen (pOp->apPropValues[i]->u.zString)
                                    : 0;
              size_t nMaxSanitized
                  = nOrigLen * 2 + 1; /* Worst case: all chars escaped */
              nNeeded = snprintf (NULL, 0, "%s\"%s\":\"\"", i > 0 ? "," : "",
                                  pOp->azPropNames[i])
                        + nMaxSanitized;
            }
          else if (pOp->apPropValues[i]->type == CYPHER_VALUE_INTEGER)
          ...
          /* @SELF_REVIEW.md Section 5.1: Add property with security
           * sanitization */
          if (pOp->apPropValues[i]->type == CYPHER_VALUE_STRING)
            {
              /* Sanitize string values against injection attacks */
              char *zSanitizedValue
                  = cypherSanitizeString (pOp->apPropValues[i]->u.zString);
              if (!zSanitizedValue)
                {
                  sqlite3_free (zProps);
              ...
              nUsed += snprintf (zProps + nUsed, nAlloc - nUsed,
                                 "%s\"%s\":\"%s\"", i > 0 ? "," : "",
                                 pOp->azPropNames[i], zSanitizedValue);
  ...
  pWriteOp->zRelType
      = sqlite3_mprintf ("%s", pOp->zRelType ? pOp->zRelType : "");
  ...
  rc = cypherStorageAddEdge (pCtx->pGraph, pOp->iCreatedRelId,
                             pOp->iFromNodeId, pOp->iToNodeId, pOp->zRelType,
                             1.0, /* Default weight */
                             zPropsJson);
  if (rc != SQLITE_OK)
    {
...
*/
int
cypherValidateNodeExists (CypherWriteContext *pCtx, sqlite3_int64 iNodeId)
{
  int bExists;
...
*/
int
cypherNodeMatches (CypherWriteContext *pCtx, sqlite3_int64 iNodeId,
                   char **azLabels, int nLabels, char **azProps,
                   CypherValue **apValues, int nProps)
{
  ...
  if (nLabels > 0)
    {
      zSql = sqlite3_mprintf ("SELECT 1 FROM %s WHERE id = %lld",
                              pCtx->pGraph->zNodeTableName, iNodeId);

      for (i = 0; i < nLabels; i++)
        {
          /* Use %q to properly escape the label for SQL LIKE pattern */
          char *zNewSql = sqlite3_mprintf (
              "%s AND json_extract(labels, '$') LIKE '%%\"%q\"%%'", zSql,
              azLabels[i]);
          sqlite3_free (zSql);
          zSql = zNewSql;
          if (!zSql)
  ...
  for (i = 0; i < nProps && bMatches; i++)
    {
      ...
      zSql = sqlite3_mprintf (
          "SELECT 1 FROM %s WHERE id = %lld "
          "AND json_extract(properties, '$.%s') = json('%s')",
          pCtx->pGraph->zNodeTableName, iNodeId, azProps[i], zValueJson);
...
*/
sqlite3_int64
cypherFindMatchingNode (CypherWriteContext *pCtx, char **azLabels, int nLabels,
  ...
  if (nLabels > 0)
    {
      ...
      zSql = sqlite3_mprintf ("SELECT id FROM %s WHERE "
                              "json_extract(labels, '$[0]') = '%s'",
                              pCtx->pGraph->zNodeTableName, azLabels[0]);

      /* Add additional label filters */
      for (i = 1; i < nLabels; i++)
        {
          char *zNewSql
              = sqlite3_mprintf ("%s AND json_extract(labels, '$[%d]') = '%s'",
                                 zSql, i, azLabels[i]);
          sqlite3_free (zSql);
          zSql = zNewSql;
        }
  ...
  else
    {
      ...
      zSql = sqlite3_mprintf ("SELECT id FROM %s",
                              pCtx->pGraph->zNodeTableName);
    }
  ...
  for (i = 0; i < nProps; i++)
    {
      ...
      if (zValueJson)
        {
          /* Compare against the literal JSON representation directly */
          char *zNewSql = sqlite3_mprintf (
              "%s AND json_extract(properties, '$.%s') = %s", zSql,
...
*/
char *
cypherGetNodeRelationships (CypherWriteContext *pCtx, sqlite3_int64 iNodeId)
{
  ...
  zSql = sqlite3_mprintf ("SELECT edge_id FROM graph_edges WHERE from_node = "
                          "%lld OR to_node = %lld",
                          iNodeId, iNodeId);
      ...
      while ((rc = sqlite3_step (pStmt)) == SQLITE_ROW)
        {
          ...
          int nNeeded
              = snprintf (NULL, 0, "%s%lld", bFirst ? "" : ",", iEdgeId);
          if (nUsed + nNeeded + 2 >= nAlloc)
            {
              nAlloc = (nUsed + nNeeded + 256) * 2;
          ...
          nUsed += snprintf (zResult + nUsed, nAlloc - nUsed, "%s%lld",
                             bFirst ? "" : ",", iEdgeId);
          bFirst = 0;
        }
      sqlite3_finalize (pStmt);
...
*/
int
cypherExecuteOperations (CypherWriteContext *pCtx)
{
  CypherWriteOp *pOp;
  int rc = SQLITE_OK;
  ...
  for (pOp = pCtx->pOperations; pOp; pOp = pOp->pNext)
    {
      switch (pOp->type)
        ...
        case CYPHER_WRITE_CREATE_NODE:
          ...
          rc = cypherStorageAddNode (pCtx->pGraph, pOp->iNodeId, NULL, 0,
                                     NULL);
          if (rc != SQLITE_OK)
            return rc;
          break;
        ...
        case CYPHER_WRITE_CREATE_RELATIONSHIP:
          ...
          rc = cypherStorageAddEdge (pCtx->pGraph, pOp->iRelId, pOp->iFromId,
                                     pOp->iToId, pOp->zRelType, 1.0, NULL);
          if (rc != SQLITE_OK)
            return rc;
          break;
        ...
        case CYPHER_WRITE_MERGE_NODE:
          ...
          {
            sqlite3_int64 iFoundId = cypherFindMatchingNode (
                pCtx, NULL, 0, NULL, NULL, 0); /* Basic node search */
        ...
        case CYPHER_WRITE_SET_PROPERTY:
          ...
          rc = cypherStorageUpdateProperties (pCtx->pGraph, pOp->iNodeId,
                                              pOp->iRelId, pOp->zProperty,
                                              pOp->pNewValue);
          if (rc != SQLITE_OK)
            return rc;
          break;
        ...
        case CYPHER_WRITE_SET_LABEL:
          ...
          /* For label updates, we need to read current labels, add new ones,
           * and update */
          {
            char *zSql = sqlite3_mprintf (
...
*/
int
cypherRollbackOperations (CypherWriteContext *pCtx)
{
  CypherWriteOp *pOp;
  int rc = SQLITE_OK;
  ...
  for (int i = nOps - 1; i >= 0; i--)
    {
      ...
      switch (pOp->type)
        ...
        case CYPHER_WRITE_SET_PROPERTY:
          ...
          if (pOp->pOldValue)
            {
              cypherStorageUpdateProperties (pCtx->pGraph, pOp->iNodeId,
                                             pOp->iRelId, pOp->zProperty,
                                             pOp->pOldValue);
            }
          break;
          ...
          if (pOp->zOldLabels)
            {
              ...
              if (zRestoreSql)
                {
                  sqlite3_int64 rowId;
                  cypherStorageExecuteUpdate (pCtx->pGraph, zRestoreSql,
                                              &rowId);
        ...
        case CYPHER_WRITE_MERGE_NODE:
          ...
          if (pOp->iNodeId > 0)
            {
              cypherStorageDeleteNode (pCtx->pGraph, pOp->iNodeId,
                                       1); /* Force detach */
            }
          break;
        ...
        case CYPHER_WRITE_DELETE_RELATIONSHIP:
          ...
          if (pOp->type == CYPHER_WRITE_DELETE_NODE
              || pOp->type == CYPHER_WRITE_DETACH_DELETE_NODE)
            {
              ...
              if (pOp->zOldLabels && pOp->pOldValue)
                {
                  ...
                  if (zValueStr)
                    {
                      cypherStorageAddNode (pCtx->pGraph, pOp->iNodeId, NULL,
                                            0, zValueStr);
          ...
          else if (pOp->type == CYPHER_WRITE_DELETE_RELATIONSHIP)
            {
              ...
              if (pOp->iFromId > 0 && pOp->iToId > 0)
                {
                  char *zValueStr = pOp->pOldValue
                                        ? cypherValueToString (pOp->pOldValue)
                                        : NULL;
...
*/
CypherWriteOp *
cypherWriteOpCreate (CypherWriteOpType type)
{
  CypherWriteOp *pOp
      = (CypherWriteOp *)sqlite3_malloc (sizeof (CypherWriteOp));
  if (!pOp)
    return NULL;
...
*/
void
cypherWriteOpDestroy (CypherWriteOp *pOp)
{
  if (!pOp)
    return;
...
*/
void
cypherCreateNodeOpDestroy (CreateNodeOp *pOp)
{
  int i;
...
*/
void
cypherCreateRelOpDestroy (CreateRelOp *pOp)
{
  int i;
...
*/
int
cypherMergeNode (CypherWriteContext *pCtx, MergeNodeOp *pOp)
{
  ...
  iFoundNodeId = cypherFindMatchingNode (pCtx, pOp->azLabels, pOp->nLabels,
                                         pOp->azMatchProps, pOp->apMatchValues,
                                         pOp->nMatchProps);
  ...
  if (iFoundNodeId > 0)
  ...
  else
    {
      ...
      /* Use storage-backed next id to avoid collisions within the DB */
      pOp->iNodeId = cypherStorageGetNextNodeId (pCtx->pGraph);
      pOp->bWasCreated = 1;
      ...
      for (i = 0; i < pOp->nMatchProps; i++)
        {
          ...
          if (pOp->apMatchValues[i]->type == CYPHER_VALUE_STRING)
            {
              nNeeded = snprintf (
                  NULL, 0, "%s\"%s\":\"%s\"", propCount > 0 ? "," : "",
                  pOp->azMatchProps[i], pOp->apMatchValues[i]->u.zString);
            }
          else if (pOp->apMatchValues[i]->type == CYPHER_VALUE_INTEGER)
            {
              nNeeded = snprintf (
                  NULL, 0, "%s\"%s\":%lld", propCount > 0 ? "," : "",
                  pOp->azMatchProps[i], pOp->apMatchValues[i]->u.iInteger);
            }
          else
            {
              nNeeded
                  = snprintf (NULL, 0, "%s\"%s\":null",
      ...
      if (rc != SQLITE_OK)
        {
          /* pWriteOp owns zLabelsJson now; do not free it here */
          cypherWriteOpDestroy (pWriteOp);
          sqlite3_free (zPropsJson);
          return rc;
        }
...
GDBG("MERGE create node id=%lld labels=%s props=%s\n",
               (long long)pOp->iNodeId, zLabelsJson ? zLabelsJson : "[]",
               zPropsJson ? zPropsJson : "{}");
      rc = cypherStorageAddNode (pCtx->pGraph, pOp->iNodeId, azLabelPtrs,
                                 pOp->nLabels, zPropsJson);
      if (rc != SQLITE_OK)
        {
          /* zLabelsJson is owned by pWriteOp in the context op log; do not free */
          sqlite3_free (zPropsJson);
          return rc;
        }
...
** NOTE: Initial stub - relationship MERGE implementation will be completed in
*subsequent steps.
*/
static int cypherResolveVarProperty(CypherWriteContext *pCtx, const char *zRef, CypherValue *pOut) {
  /* zRef format: var.prop */
  if (!pCtx || !zRef || !pOut) return SQLITE_MISUSE;
  const char *dot = strchr(zRef, '.');
  if (!dot) return SQLITE_MISUSE;
  int nVar = (int)(dot - zRef);
  if (nVar <= 0) return SQLITE_MISUSE;
  char *zVar = sqlite3_mprintf("%.*s", nVar, zRef);
  const char *zProp = dot + 1;
  if (!zVar || !zProp || !zProp[0]) { sqlite3_free(zVar); return SQLITE_MISUSE; }
  CypherValue *pVarVal = executionContextGet(pCtx->pExecContext, zVar);
  const char *zTable = NULL;
  sqlite3_int64 iId = 0;
  if (pVarVal) {
    if (pVarVal->type == CYPHER_VALUE_NODE) { zTable = pCtx->pGraph->zNodeTableName; iId = pVarVal->u.iNodeId; }
    else if (pVarVal->type == CYPHER_VALUE_RELATIONSHIP) { zTable = pCtx->pGraph->zEdgeTableName; iId = pVarVal->u.iRelId; }
  }
  sqlite3_free(zVar);
  if (!zTable) return SQLITE_MISUSE;
  sqlite3_stmt *st = NULL;
  char *zSql = sqlite3_mprintf("SELECT json_extract(properties, '$.%q'), json_type(properties, '$.%q') FROM %s WHERE id = %lld", zProp, zProp, zTable, iId);
  if (!zSql) return SQLITE_NOMEM;
  int rc = sqlite3_prepare_v2(pCtx->pGraph->pDb, zSql, -1, &st, NULL);
  sqlite3_free(zSql);
  if (rc != SQLITE_OK) return rc;
  rc = sqlite3_step(st);
  if (rc != SQLITE_ROW) { sqlite3_finalize(st); cypherValueSetNull(pOut); return SQLITE_OK; }
  const unsigned char *zType = sqlite3_column_text(st, 1);
  int colType = sqlite3_column_type(st, 0);
  if (zType) {
    if (sqlite3_stricmp((const char*)zType, "integer") == 0 && colType == SQLITE_INTEGER) {
      cypherValueSetInteger(pOut, sqlite3_column_int64(st, 0));
    } else if (sqlite3_stricmp((const char*)zType, "real") == 0 && (colType == SQLITE_FLOAT || colType == SQLITE_INTEGER)) {
      cypherValueSetFloat(pOut, sqlite3_column_double(st, 0));
    } else if (sqlite3_stricmp((const char*)zType, "text") == 0 && colType == SQLITE_TEXT) {
      const unsigned char *zTxt = sqlite3_column_text(st, 0);
      cypherValueSetString(pOut, (const char*)zTxt);
    } else if (sqlite3_stricmp((const char*)zType, "true") == 0) {
      cypherValueSetBoolean(pOut, 1);
    } else if (sqlite3_stricmp((const char*)zType, "false") == 0) {
      cypherValueSetBoolean(pOut, 0);
    } else {
      cypherValueSetNull(pOut);
    }
  } else {
    cypherValueSetNull(pOut);
  }
...
static int cypherResolveVarPropertyWithFallback(CypherWriteContext *pCtx, const char *zRef,
                                               const char *zFromVar, sqlite3_int64 iFromId,
                                               const char *zToVar, sqlite3_int64 iToId,
                                               CypherValue *pOut) {
  if (!pCtx || !zRef || !pOut) return SQLITE_MISUSE;
  const char *dot = strchr(zRef, '.');
  if (!dot) return SQLITE_MISUSE;
  ...
  const char *zProp = dot + 1;
  if (!zVar || !zProp || !zProp[0]) { sqlite3_free(zVar); return SQLITE_MISUSE; }
  const char *zTable = NULL; sqlite3_int64 iId = 0;
  if (zFromVar && sqlite3_stricmp(zVar, zFromVar) == 0) { zTable = pCtx->pGraph->zNodeTableName; iId = iFromId; }
  else if (zToVar && sqlite3_stricmp(zVar, zToVar) == 0) { zTable = pCtx->pGraph->zNodeTableName; iId = iToId; }
  sqlite3_free(zVar);
  if (!zTable || iId <= 0) {
    /* fallback to generic resolution via execution context */
    return cypherResolveVarProperty(pCtx, zRef, pOut);
  }
  sqlite3_stmt *st = NULL;
  char *zSql = sqlite3_mprintf("SELECT json_extract(properties, '$.%q'), json_type(properties, '$.%q') FROM %s WHERE id = %lld", zProp, zProp, zTable, iId);
  if (!zSql) return SQLITE_NOMEM;
...
int
cypherMergeRelationship (CypherWriteContext *pCtx, MergeRelOp *pOp)
{
  int rc = SQLITE_OK;
  sqlite3_stmt *pStmt = NULL;
  int i;

  if (!pCtx || !pOp || !pCtx->pGraph || !pCtx->pExecContext)
    return SQLITE_MISUSE;

  /* Resolve endpoint node IDs from bound variables */
  if ((!pOp->iFromId || !pOp->iToId) && (pOp->zFromVar || pOp->zToVar))
    {
      CypherValue *vFrom
          = pOp->zFromVar
                ? executionContextGet (pCtx->pExecContext, pOp->zFromVar)
                : NULL;
      CypherValue *vTo
          = pOp->zToVar ? executionContextGet (pCtx->pExecContext, pOp->zToVar)
                        : NULL;
GDBG("cypherMergeRelationship: lookup fromVar=%s -> %s, toVar=%s -> %s\n",
               pOp->zFromVar ? pOp->zFromVar : "(null)",
               (vFrom && vFrom->type == CYPHER_VALUE_NODE) ? "node" : "(missing)",
               pOp->zToVar ? pOp->zToVar : "(null)",
               (vTo && vTo->type == CYPHER_VALUE_NODE) ? "node" : "(missing)");
      if (!vFrom || vFrom->type != CYPHER_VALUE_NODE || !vTo
          || vTo->type != CYPHER_VALUE_NODE)
        {
          return SQLITE_MISUSE;
        }
      pOp->iFromId = vFrom->u.iNodeId;
      pOp->iToId = vTo->u.iNodeId;
    }
  if (pOp->iFromId <= 0 || pOp->iToId <= 0)
    return SQLITE_MISUSE;

  /* Verify endpoints exist */
  rc = cypherValidateNodeExists (pCtx, pOp->iFromId);
  if (rc != SQLITE_OK)
    return rc;
  rc = cypherValidateNodeExists (pCtx, pOp->iToId);
  if (rc != SQLITE_OK)
    return rc;

  /* Build SQL to find matching relationship deterministically (lowest edge_id)
   */
  {
    char *zSql = NULL;
    char *zTypeCond = NULL;
    if (pOp->zRelType && pOp->zRelType[0])
      {
        zTypeCond = sqlite3_mprintf (" AND edge_type = '%q'", pOp->zRelType);
      }
    else
      {
        zTypeCond = sqlite3_mprintf ("");
      }

    /* Base match from->to */
    zSql = sqlite3_mprintf ("SELECT id, properties FROM %s WHERE source = "
                            "%lld AND target = %lld%s",
                            pCtx->pGraph->zEdgeTableName, pOp->iFromId,
                            pOp->iToId, zTypeCond ? zTypeCond : "");

    /* Add property equality predicates if any */
    if (pOp->nMatchProps > 0)
      {
        for (i = 0; i < pOp->nMatchProps; i++)
          {
            char *zValJson = cypherValueToJson (pOp->apMatchValues[i]);
            const char *zCmp = zValJson ? zValJson : "null";
            char *zNew = sqlite3_mprintf (
                "%s AND json_extract(properties, '$.%q') = %s", zSql,
                pOp->azMatchProps[i], zCmp);
            sqlite3_free (zValJson);
            sqlite3_free (zSql);
            zSql = zNew;
          }
      }
    /* Deterministic pick */
    char *zFinal = sqlite3_mprintf ("%s ORDER BY id ASC LIMIT 1", zSql);
    sqlite3_free (zSql);
    zSql = zFinal;
    ...
    if (rc == SQLITE_ROW)
      {
        /* Found match */
        pOp->iRelId = sqlite3_column_int64 (pStmt, 0);
GDBG("MERGE rel matched edge id=%lld; onMatchProps=%d\n", (long long)pOp->iRelId, pOp->nOnMatchProps);
        sqlite3_finalize (pStmt);
        pStmt = NULL;
        pOp->bWasCreated = 0;
        /* Apply ON MATCH property updates */
        for (i = 0; i < pOp->nOnMatchProps; i++)
          {
            const CypherValue *pVal = pOp->apOnMatchValues[i];
            CypherValue eval;
            int useEval = 0;
GDBG("ON MATCH prop %d: name=%s value_type=%d value_str=%s\n", i, pOp->azOnMatchProps[i] ? pOp->azOnMatchProps[i] : "null", pVal ? pVal->type : -1, pVal && pVal->type == CYPHER_VALUE_STRING && pVal->u.zString ? pVal->u.zString : "(non-string)");
            if (pVal && pVal->type == CYPHER_VALUE_STRING && pVal->u.zString && strchr(pVal->u.zString, '.') != NULL) {
              cypherValueInit(&eval);
              rc = cypherResolveVarPropertyWithFallback(pCtx, pVal->u.zString,
                                                        pOp->zFromVar, pOp->iFromId,
                                                        pOp->zToVar, pOp->iToId,
                                                        &eval);
GDBG("Resolved %s -> type=%d int=%lld\n", pVal->u.zString, eval.type, (long long)(eval.type == CYPHER_VALUE_INTEGER ? eval.u.iInteger : -999));
              if (rc == SQLITE_OK) {
                useEval = 1;
              }
            }
            rc = cypherStorageUpdateProperties (pCtx->pGraph, 0, pOp->iRelId,
                                                pOp->azOnMatchProps[i],
                                                useEval ? &eval : pOp->apOnMatchValues[i]);
            if (useEval) { cypherValueDestroy(&eval); }
            if (rc != SQLITE_OK)
              return rc;
          }
        /* Debug: show updated score */
        {
          sqlite3_stmt *pDbg = NULL; char *zDbg = sqlite3_mprintf("SELECT json_extract(properties,'$.score') FROM %s WHERE id=%lld", pCtx->pGraph->zEdgeTableName, pOp->iRelId);
          if (sqlite3_prepare_v2(pCtx->pGraph->pDb, zDbg, -1, &pDbg, NULL) == SQLITE_OK) {
            if (sqlite3_step(pDbg) == SQLITE_ROW) {
GDBG("Updated score now=%s\n", (const char*)sqlite3_column_text(pDbg, 0));
            }
            sqlite3_finalize(pDbg);
          }
          sqlite3_free(zDbg);
        }
        /* Log merge relationship op */
        CypherWriteOp *pWriteOp
            = cypherWriteOpCreate (CYPHER_WRITE_MERGE_RELATIONSHIP);
        if (pWriteOp)
          {
            pWriteOp->iRelId = pOp->iRelId;
            pWriteOp->iFromId = pOp->iFromId;
            pWriteOp->iToId = pOp->iToId;
            pWriteOp->zRelType
                = pOp->zRelType ? sqlite3_mprintf ("%s", pOp->zRelType) : NULL;
            cypherWriteContextAddOperation (pCtx, pWriteOp);
          }
        /* Bind relationship variable, if any */
        if (pOp->zRelVar)
          {
            CypherValue relVal;
            cypherValueInit (&relVal);
            cypherValueSetRelationship (&relVal, pOp->iRelId);
            executionContextBind (pCtx->pExecContext, pOp->zRelVar, &relVal);
            cypherValueDestroy (&relVal);
          }
        return SQLITE_OK;
      }
    sqlite3_finalize (pStmt);
    ...
    /* If not found and pattern is undirected, also try reverse */
    if (pOp->bUndirected)
      {
        zSql = sqlite3_mprintf ("SELECT id, properties FROM %s WHERE source = "
                                "%lld AND target = %lld%s",
                                pCtx->pGraph->zEdgeTableName, pOp->iToId,
                                pOp->iFromId, zTypeCond ? zTypeCond : "");
        if (pOp->nMatchProps > 0)
          {
            for (i = 0; i < pOp->nMatchProps; i++)
              {
                ...
                const char *zCmp = zValJson ? zValJson : "null";
                char *zNew = sqlite3_mprintf (
                    "%s AND json_extract(properties, '$.%q') = %s",
                    zSql, pOp->azMatchProps[i], zCmp);
                sqlite3_free (zValJson);
                sqlite3_free (zSql);
                zSql = zNew;
              }
          }
        zFinal = sqlite3_mprintf ("%s ORDER BY id ASC LIMIT 1", zSql);
        sqlite3_free (zSql);
        zSql = zFinal;
        rc = sqlite3_prepare_v2 (pCtx->pGraph->pDb, zSql, -1, &pStmt, NULL);
        sqlite3_free (zSql);
        ...
        if (rc == SQLITE_ROW)
          {
            pOp->iRelId = sqlite3_column_int64 (pStmt, 0);
GDBG("MERGE rel matched (reverse) edge id=%lld; onMatchProps=%d\n", (long long)pOp->iRelId, pOp->nOnMatchProps);
            sqlite3_finalize (pStmt);
            pStmt = NULL;
            pOp->bWasCreated = 0;
            /* Apply ON MATCH properties */
            for (i = 0; i < pOp->nOnMatchProps; i++)
              {
                rc = cypherStorageUpdateProperties (
                    pCtx->pGraph, 0, pOp->iRelId, pOp->azOnMatchProps[i],
                    pOp->apOnMatchValues[i]);
                if (rc != SQLITE_OK)
                  return rc;
              }
            /* Debug: show updated score */
...
GDBG("Updated score now=%s\n", (const char*)sqlite3_column_text(pDbg, 0));
            ...
            if (pWriteOp)
              {
                ...
                pWriteOp->iToId = pOp->iToId;
                pWriteOp->zRelType
                    = pOp->zRelType ? sqlite3_mprintf ("%s", pOp->zRelType)
                                    : NULL;
                cypherWriteContextAddOperation (pCtx, pWriteOp);
              }
            if (pOp->zRelVar)
              {
                ...
                cypherValueInit (&relVal);
                cypherValueSetRelationship (&relVal, pOp->iRelId);
                executionContextBind (pCtx->pExecContext, pOp->zRelVar,
                                      &relVal);
                cypherValueDestroy (&relVal);
              }
            return SQLITE_OK;
    ...
    sqlite3_free (zTypeCond);
  }

  /* No matching relationship; create new one with merged properties (match +
   * on create) */
  ...
  for (i = 0; i < pOp->nMatchProps; i++)
    {
      char *zValJson = cypherValueToJson (pOp->apMatchValues[i]);
      int nNeeded
          = snprintf (NULL, 0, "%s\"%s\":%s", propCount > 0 ? "," : "",
                      pOp->azMatchProps[i], zValJson ? zValJson : "null");
      if (nUsed + nNeeded + 2 >= nAlloc)
        {
          nAlloc = (nUsed + nNeeded + 256) * 2;
          char *zNew = sqlite3_realloc (zProps, nAlloc);
      ...
      nUsed += snprintf (zProps + nUsed, nAlloc - nUsed, "%s\"%s\":%s",
                         propCount > 0 ? "," : "", pOp->azMatchProps[i],
                         zValJson ? zValJson : "null");
      sqlite3_free (zValJson);
      propCount++;
    }
  for (i = 0; i < pOp->nOnCreateProps; i++)
    {
      /* Evaluate var.prop references at runtime */
      const CypherValue *pValIn = pOp->apOnCreateValues[i];
      CypherValue eval;
      const CypherValue *pValUse = pValIn;
      if (pValIn && pValIn->type == CYPHER_VALUE_STRING && pValIn->u.zString && strchr(pValIn->u.zString, '.') != NULL)
        {
          cypherValueInit(&eval);
          if (cypherResolveVarPropertyWithFallback(pCtx, pValIn->u.zString,
                                                   pOp->zFromVar, pOp->iFromId,
                                                   pOp->zToVar, pOp->iToId,
                                                   &eval) == SQLITE_OK)
            {
      ...
      char *zValJson = cypherValueToJson (pValUse);
      int nNeeded
          = snprintf (NULL, 0, "%s\"%s\":%s", propCount > 0 ? "," : "",
                      pOp->azOnCreateProps[i], zValJson ? zValJson : "null");
      if (nUsed + nNeeded + 2 >= nAlloc)
        {
          nAlloc = (nUsed + nNeeded + 256) * 2;
          char *zNew = sqlite3_realloc (zProps, nAlloc);
          if (!zNew)
            {
              sqlite3_free (zProps);
              if (pValUse == &eval) cypherValueDestroy(&eval);
              return SQLITE_NOMEM;
            }
          zProps = zNew;
        }
  ...
  rc = cypherStorageAddEdge (pCtx->pGraph, pOp->iRelId, pOp->iFromId,
                             pOp->iToId, pOp->zRelType, 1.0, zProps);
  sqlite3_free (zProps);
  if (rc != SQLITE_OK)
    return rc;

  /* Apply ON MATCH props do nothing on create; bind variable */
  pOp->bWasCreated = 1;
  if (pOp->zRelVar)
    {
      CypherValue relVal;
...
*/
CypherWriteIterator *
cypherMergeNodeIteratorCreate (CypherWriteContext *pCtx, MergeNodeOp *pOp)
{
  ...
  pIterator
      = (CypherWriteIterator *)sqlite3_malloc (sizeof (CypherWriteIterator));
  if (!pIterator)
    return NULL;
...
void
cypherMergeNodeOpDestroy (MergeNodeOp *pOp)
{
  int i;
...
*/
int
cypherSetProperty (CypherWriteContext *pCtx, SetPropertyOp *pOp)
{
  CypherWriteOp *pWriteOp;
  int rc = SQLITE_OK;
  ...
  sqlite3_stmt *pStmt = NULL;
  char *zSql = sqlite3_mprintf ("SELECT json_extract(properties, '$.%s') FROM "
                                "graph_nodes WHERE node_id = %lld",
  ...
  if (pOp->pValue->type == CYPHER_VALUE_STRING && pOp->pValue->u.zString)
    {
      pWriteOp->pNewValue->u.zString
          = sqlite3_mprintf ("%s", pOp->pValue->u.zString);
  ...
  rc = cypherStorageUpdateProperties (
      pCtx->pGraph, pOp->iNodeId, 0, /* No relation ID for node properties */
      pOp->zProperty, pOp->pValue);
  if (rc != SQLITE_OK)
    {
      fprintf (stderr,
               "cypherSetProperty: update failed rc=%d for node %lld prop=%s\n",
               rc, (long long)pOp->iNodeId, pOp->zProperty ? pOp->zProperty : "(null)");
      return rc;
    }
...
*/
int
cypherSetLabel (CypherWriteContext *pCtx, SetLabelOp *pOp)
{
  CypherWriteOp *pWriteOp;
  char *zLabelsJson = NULL;
  int rc = SQLITE_OK;
  ...
  sqlite3_stmt *pStmt = NULL;
  char *zSql = sqlite3_mprintf (
      "SELECT labels FROM graph_nodes WHERE node_id = %lld", pOp->iNodeId);
  ...
  char *zUpdateSql = sqlite3_mprintf (
      "UPDATE graph_nodes SET labels = '%s' WHERE node_id = %lld", zLabelsJson,
      pOp->iNodeId);
...
*/
CypherWriteIterator *
cypherSetPropertyIteratorCreate (CypherWriteContext *pCtx, SetPropertyOp *pOp)
{
  CypherWriteIterator *pIterator;
...
*/
CypherWriteIterator *
cypherSetLabelIteratorCreate (CypherWriteContext *pCtx, SetLabelOp *pOp)
{
  CypherWriteIterator *pIterator;
...
SetPropertyOp *
cypherSetPropertyOpCreate (void)
{
  SetPropertyOp *pOp
      = (SetPropertyOp *)sqlite3_malloc (sizeof (SetPropertyOp));
  if (!pOp)
    return NULL;
...
void
cypherSetPropertyOpDestroy (SetPropertyOp *pOp)
{
  if (!pOp)
    return;
...
void
cypherSetLabelOpDestroy (SetLabelOp *pOp)
{
  int i;
...
*/
int
cypherDelete (CypherWriteContext *pCtx, DeleteOp *pOp)
{
  CypherWriteOp *pWriteOp;
  ...
  if (pOp->bIsNode)
  ...
  else
    {
      ...
      sqlite3_stmt *pStmt = NULL;
      char *zSql = sqlite3_mprintf (
          "SELECT from_node, to_node, edge_type, weight, properties FROM "
          "graph_edges WHERE edge_id = %lld",
          pOp->iRelId);
...
*/
CypherWriteIterator *
cypherDeleteIteratorCreate (CypherWriteContext *pCtx, DeleteOp *pOp)
{
  CypherWriteIterator *pIterator;
...
void
cypherDeleteOpDestroy (DeleteOp *pOp)
{
  if (!pOp)
    return;
...
*/
CypherWriteIterator *
cypherCreateNodeIteratorCreate (CypherWriteContext *pCtx, CreateNodeOp *pOp)
{
  CypherWriteIterator *pIterator;
...
*/
CypherWriteIterator *
cypherCreateRelIteratorCreate (CypherWriteContext *pCtx, CreateRelOp *pOp)
{
  CypherWriteIterator *pIterator;
