...
static int
mergeValueLooksNumeric (const char *zValue)
{
  const char *z = zValue;
  int seenDigit = 0;
...
static char *
mergeRenderExpressionJson (CypherAst *pExpr)
{
  const char *zValue;
  ...
  if (mergeValueLooksNumeric (zValue) || strcmp (zValue, "true") == 0
      || strcmp (zValue, "false") == 0 || strcmp (zValue, "null") == 0)
    {
...
static char *
mergeSerializeMatchProps (MergeClauseDetails *pDetails)
{
  char *zJson;
  int first = 1;
  int i;
  ...
  for (i = 0; i < pDetails->nMatchProps; i++)
    {
      ...
      zNext = sqlite3_mprintf ("%s%s\"%s\":%s", zJson, first ? "" : ",",
...
static char *
mergeSerializeSetOps (MergeSetOp *aOps, int nOps)
{
  char *zJson;
  int first = 1;
  int i;
  ...
  for (i = 0; i < nOps; i++)
    {
      ...
      zNext = sqlite3_mprintf (
          "%s%s{\"target\":\"%s\",\"property\":\"%s\",\"value\":%s}", zJson,
          first ? "" : ",", pOp->zTargetVar ? pOp->zTargetVar : "",
          pOp->zProperty ? pOp->zProperty : "", zValueJson);
      sqlite3_free (zJson);
      sqlite3_free (zValueJson);
      if (!zNext)
        return NULL;
...
PhysicalPlanNode *
physicalPlanNodeCreate (PhysicalOperatorType type)
{
...
*/
void
physicalPlanNodeDestroy (PhysicalPlanNode *pNode)
{
  int i;
...
*/
int
physicalPlanNodeAddChild (PhysicalPlanNode *pParent, PhysicalPlanNode *pChild)
{
...
*/
const char *
physicalOperatorTypeName (PhysicalOperatorType type)
{
  switch (type)
    {
...
*/
PhysicalPlanNode *
logicalPlanToPhysical (LogicalPlanNode *pLogical, PlanContext *pContext)
{
  ...
  switch (pLogical->type)
          ...
          if (pPhysical)
            {
              ...
              pPhysical->rCost
                  = pLogical->rEstimatedCost * pContext->rIndexCostFactor;
            }
        }
      else
        {
      ...
      if (pPhysical && pLogical->zProperty)
        {
          ...
          pPhysical->zValue = sqlite3_mprintf (
              "%s", pLogical->zValue ? pLogical->zValue : "");
          pPhysical->rCost
              = pLogical->rEstimatedCost * 0.1; /* Index is much faster */
        }
      break;
      ...
      if (pPhysical)
        {
          if (pLogical->zProperty)
            {
              pPhysical->zProperty
                  = sqlite3_mprintf ("%s", pLogical->zProperty);
            }
          if (pLogical->zValue)
            {
          ...
          /* Copy filter AST from logical plan's pExtra to physical plan's
           * pExecState */
          if (pLogical->pExtra)
            {
    ...
    case LOGICAL_CARTESIAN_PRODUCT:
      /* Cartesian product uses nested loop join without join condition */
    ...
    case LOGICAL_SKIP:
      ...
      /* For now, map to LIMIT - full implementation would need separate SKIP
       * physical operator */
    ...
    case LOGICAL_CREATE:
      ...
      /* Copy pattern AST from logical plan's pExtra to physical plan's pExecState */
      if (pPhysical && pLogical->pExtra)
        {
          pPhysical->pExecState = pLogical->pExtra;
        }
      /* Also copy alias, label, and property for simple nodes */
      if (pPhysical && pLogical->zAlias)
        {
          pPhysical->zAlias = sqlite3_mprintf ("%s", pLogical->zAlias);
        }
      if (pPhysical && pLogical->zLabel)
        {
          pPhysical->zLabel = sqlite3_mprintf ("%s", pLogical->zLabel);
        }
      if (pPhysical && pLogical->zProperty)
        ...
        if (pPhysical && pDetails)
          {
            ...
            pPhysical->zOnCreateJson = mergeSerializeSetOps (
                pDetails->aOnCreateOps, pDetails->nOnCreateOps);
            pPhysical->zOnMatchJson = mergeSerializeSetOps (
                pDetails->aOnMatchOps, pDetails->nOnMatchOps);
    ...
    case LOGICAL_DETACH_DELETE:
      pPhysical = physicalPlanNodeCreate (PHYSICAL_DETACH_DELETE);
      break;
  ...
  /* Copy extra data (e.g., pattern AST for CREATE) - but only if not already
   * set */
...
*/
char *
physicalPlanToString (PhysicalPlanNode *pNode)
{
  char *zResult;
  char *zChildren = NULL;
  char *zDetails = NULL;
  ...
  if (pNode->zIndexName)
  ...
  else if (pNode->zProperty)
    {
      if (pNode->zValue)
        {
          zDetails = sqlite3_mprintf ("prop=%s val=%s", pNode->zProperty,
                                      pNode->zValue);
        }
      else
        {
  ...
  if (pNode->zAlias && zDetails)
    {
      zResult = sqlite3_mprintf ("%s(%s %s cost=%.1f rows=%lld%s%s)",
                                 ...
                                 pNode->zAlias, zDetails, pNode->rCost,
                                 pNode->iRows, zChildren ? " [" : "",
                                 zChildren ? zChildren : "");
    }
  else if (pNode->zAlias)
    {
      zResult = sqlite3_mprintf (
          "%s(%s cost=%.1f rows=%lld%s%s)",
          physicalOperatorTypeName (pNode->type), pNode->zAlias, pNode->rCost,
          pNode->iRows, zChildren ? " [" : "", zChildren ? zChildren : "");
    }
  else if (zDetails)
    {
      zResult = sqlite3_mprintf (
          "%s(%s cost=%.1f rows=%lld%s%s)",
          physicalOperatorTypeName (pNode->type), zDetails, pNode->rCost,
          pNode->iRows, zChildren ? " [" : "", zChildren ? zChildren : "");
    }
  else
    {
      zResult = sqlite3_mprintf (
          "%s(cost=%.1f rows=%lld%s%s)",
          physicalOperatorTypeName (pNode->type), pNode->rCost, pNode->iRows,
          zChildren ? " [" : "", zChildren ? zChildren : "");
    }
...
*/
PhysicalPlanNode *
cypherCreateTestPhysicalPlan (void)
{
