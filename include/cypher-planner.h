...
** These form the intermediate representation between AST and physical
*operators.
*/
typedef enum
{
...
typedef enum
{
  ...
  PHYSICAL_DETACH_DELETE, /* Detach delete nodes */
...
typedef struct PhysicalPlanNode
{
  ...
  struct PhysicalPlanNode
      *pChild; /* Primary child (for single-child operators) */
  int nChildren;
  int nChildrenAlloc;
...
/*
** Variable types for semantic validation.
*/
typedef enum
{
  /* Graph types */
...
/*
** Variable metadata for semantic validation.
*/
typedef struct VariableInfo
{
  char *zName;              /* Variable name */
  VariableType type;        /* Variable type (node, relationship, path) */
  LogicalPlanNode *pNode;   /* Logical plan node producing this variable */
  int iDefinitionLine;      /* Line where variable was first defined */
} VariableInfo;
...
typedef struct PlanContext
{
  ...
  VariableInfo **apVariableInfo; /* Enhanced variable metadata */
  int nVariableInfo;
  int nVariableInfoAlloc;
...
*/
int logicalPlanNodeAddChild (LogicalPlanNode *pParent,
...
** Returns SQLITE_OK on success, SQLITE_NOMEM on allocation failure.
*/
int physicalPlanNodeAddChild (PhysicalPlanNode *pParent,
...
*/
sqlite3_int64 logicalPlanEstimateRows (LogicalPlanNode *pNode,
...
*/
PhysicalPlanNode *logicalPlanToPhysical (LogicalPlanNode *pLogical,
                                         PlanContext *pContext);
