...
/*
** MERGE relationship operation structure.
*/
...
struct MergeRelOp
{
  ...
  char *zRelType; /* Relationship type */
  /* Match properties */
  char **azMatchProps;         /* Property names to match */
  CypherValue **apMatchValues; /* Values to match */
  int nMatchProps;             /* Number of match properties */
  ...
  int nOnMatchProps;
  sqlite3_int64 iFromId; /* Resolved source node ID */
  sqlite3_int64 iToId;   /* Resolved target node ID */
  sqlite3_int64 iRelId;  /* Matched/created relationship ID */
  int bWasCreated;       /* Whether relationship was created */
  int bUndirected;       /* Treat pattern as undirected for matching */
};
...
struct CypherWriteIterator
{
  ...
  int (*xExecute) (CypherWriteIterator *,
                   CypherResult *); /* Execute function */
};
...
*/
int cypherWriteContextAddOperation (CypherWriteContext *pCtx,
                                    CypherWriteOp *pOp);
...
/* Create a MERGE relationship iterator */
CypherWriteIterator *cypherMergeRelIteratorCreate (CypherWriteContext *pCtx,
                                                   MergeRelOp *pOp);
...
sqlite3_int64 cypherFindMatchingNode (CypherWriteContext *pCtx,
                                      ...
                                      char **azProps, CypherValue **apValues,
                                      int nProps);
...
*/
char *cypherGetNodeRelationships (CypherWriteContext *pCtx,
...
** Returns SQLITE_OK on success, error code on failure.
*/
int cypherStorageDeleteNode (GraphVtab *pGraph, sqlite3_int64 iNodeId,
                             int bDetach);
